...
bongo2020_cruise <- bongo2020 %>%
  ...
  mutate(type = "cruise",
         ...
         locationID = "http://marineregions.org/mrgid/25585")
# Confirm that all station coordinates lie within the locationID coordinates. 
...
Now that the event core has been created, next we focus on the occurrence Cores. This script contains coding for both the Occurrence Core associated with the abundance/community composition study, and the data collected on species' length and weight. We identify to the lowest common taxonomic rank where possible, and if applicable add the common name under `vernacularName` (i.e. Squid). If no taxonomic rank can be assigned to species entry (i.e. `gelatinous spp.`), we contacted the data provider to see if they can provide more taxonomic resolution, otherwise we _did not_ create records of these observations in the Occurrence Core, and therefore associate measurements to the `eventID` rather than an `occurrenceID`.  
...
Lets first create an Occurrence Core for the (estimated) taxonomic abundance data. The abundance data file also includes data from 2019 so we need to filter out only the data collected during the 2020 High Seas Expedition. Additionally, the `station` column is saved in such a way that the stations are listed as both numbers and character. As the column currently has the class 'character', station numbers are recorded as 1.0 instead of 1. We need to fix this as well. I've confirmed with Natalie Mahara (data provider) that abundance/taxonomy data is always from Net 1, unless specified in the comments. 
...
``` {r}
bongo2020_abun$identificationQualifier <- ifelse(grepl("sp.", bongo2020_abun$id), "sp. inc.", NA)
...
bongo2020_occ <- left_join(bongo2020_occ, preparations, by = "cast") %>%
  ...
  mutate(occurrenceID = paste(eventID, "zocc", row_number(), sep = ":"),
...
``` {r}
bongo2020_occ$identificationQualifier <- ifelse(grepl("sp.", bongo2020_occ$scientificname), "sp. inc.", NA)
...
bongo2020_occ_fnl <- left_join(bongo2020_occ, bongo2020_occ_worms_id, by = "scientificname") %>%
  select(eventID, stage, sex, scientificname, individualCount, identificationQualifier, kingdom, 
         phylum, class, order, family, genus, lsid, authority, rank, preparations, notes, occurrenceID, 
         occurrenceStatus) %>%
  mutate(vernacularName = case_when(
...
As a first step though, we associate the _seafloor depth_, _sampling depth_, and variables associated to the sampling event to the cast. There is still debate on whether sampling event facts need to be included, as we are essentially recording metadata in the data. Do note that that this Core standards for measurements and _facts_, and these Bongo net details are _facts_ linked to the Event. For the time being therefore, this information is recorded in this Core. It is important that the facts get linked to the correct layer.
...
Measurements or facts pertaining to the sampling effort are linked in the following chunk of code:
...
The previous three chunks of code relate to the sampling event. Next, we create the eMOF core to associate the length and weight measurements for the individuals collected within the Bongo net, particular Net 2. When the species or genus has been identified, the measurements are associated to the `occurrenceID`.  
...
# Finalize the eMOF extension, so that it includes data on the bottom depth, sampling methods, instrument parameters, length and dry weight data and finally the abundance data:  for the abundance / community composition analysis:
