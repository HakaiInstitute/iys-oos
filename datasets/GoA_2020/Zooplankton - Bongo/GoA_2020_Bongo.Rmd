...
bongo2020_abun <- left_join(bongo2020_abun, preparations, by = "eventID")
```
...
bongo2020_occ <- left_join(bongo2020_occ, bongo2020_occ_worms_id, by = "scientificname") %>%
  select(eventID, stage, sex, scientificname, individualCount, identificationQualifier, kingdom, 
         phylum, class, order, family, genus, lsid, authority, rank, preparations, notes) %>%
  mutate(vernacularName = case_when(
    scientificname == "Cephalopoda" ~ "Squid"))

bongo2020_occ_ext <- rbind(bongo2020_abun, bongo2020_occ) %>%
  ...
  mutate(
    ...
    occurrenceID = paste(eventID, "zocc", row_number(), sep = ":"),
