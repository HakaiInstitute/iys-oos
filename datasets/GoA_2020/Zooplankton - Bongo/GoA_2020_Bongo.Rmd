...
``` {r}
bongo2020_abun$identificationQualifer <- ifelse(grepl("sp.", bongo2020_abun$id), "sp. inc.", NA)
...
bongo2020_abun <- left_join(bongo2020_abun, bongo2020_worms_id, by = "scientificname") %>%
  mutate(occurrenceID = paste(cast, "bongo_occ", row_number(), sep = ":"),
...
# Join this column to the bongo2020_taxa_abundance dataframe, though first confirm with Natalie that taxonomy or abundance data is always collected from Net 1:
bongo2020_taxa_abundance <- left_join(bongo2020_taxa_abundance, preparations, by = "eventID")

# Make sure the folder path exists already (e.g. ./Zooplankton - Bongo/tidy_data)
...
bongo2020_occ <- bongo2020_occ %>%
  ...
  mutate(occurrenceID = paste(cast, `Sample ID`, sep = ":"),
         occurrenceStatus = "present")
```
...
``` {r}
bongo2020_occ$identificationQualifer <- ifelse(grepl("sp.", bongo2020_occ$scientificname), "sp. inc.", NA)
...
bongo2020_occ_core <- bongo2020_occ %>%
  select(cast, occurrenceID, occurrenceStatus, 
         ...
         authority, kingdom, 
         verbatimTaxonRank,
         phylum, class, order, family, genus, Notes, Preservation) %>%
  rename(eventID = cast,
         occurrenceRemarks = Notes,
         preparations = Preservation,
