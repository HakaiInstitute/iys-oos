...
The event Core consists of a hierarchical structure with four layers: `project`, `cruise`, `station` and `cast`. Each layer has different information associated to it. `sampleSizeValue` and `sampleSizeUnit` are not included as the volume water filtered is measured both through a flowmeter and through the estimated depth.
...
# Show column names:
print(colnames(bongo2020_event))
...
bongo2020_abun <- subset(bongo2020_taxa, date >= "2020-03-12" & date <= "2020-04-04") %>%
  mutate(Time = format(time, "%H:%M:%S"),
         ...
         # Confirmed with Natalie that abundance/taxonomy data is always from Net 1, unless specified in the comments. 
```
