...
bongo2020_taxa<- read_excel(here("Zooplankton - Bongo", "raw_data", "Bongo_taxa.xlsx"), sheet = "All data", 
                col_types = c("text", "date", "date", "numeric", "numeric", "numeric", "text", "numeric", "text", "text",
...
bongo2020$Station <- gsub(".0", "", bongo2020$Station)
bongo2020 <- bongo2020 %>%
  mutate(project = "IYS",
...
bongo2020_meta$Station <- gsub(".0", "", bongo2020_meta$Station)
bongo2020_meta <- bongo2020_meta %>%
...
bongo2020_taxa_meta <- bongo2020_taxa %>%
  mutate(Time = format(time, "%H:%M:%S"),
         eventDate = format_iso_8601(as.POSIXct(paste(date, Time),
                                                format = "%Y-%m-%d %H:%M:%S",
                                                tz = "Asia/Kamchatka")),
...
# However, the stations are listed as both numbers and character (e.g. AB). As the column currently has the class 'character', station number are recorded as 1.0 instead of 1. We need to change this:
bongo2020_taxa_meta[1] <- lapply(bongo2020_taxa_meta[1], function (x) {
  x1 <- type.convert(as.character(x), as.is = TRUE)
  ifelse(grepl("^[0-9.]+$", x1), round(as.numeric(x1)), x1)}
)

bongo2020_taxa_meta <- bongo2020_taxa_meta %>% 
  mutate(station = paste(cruise, station, sep = "_Stn:"))

bongo2020_taxa_meta$year <- as.numeric(format(as.Date(bongo2020_taxa_meta$eventDate), "%Y"))
bongo2020_taxa_meta$month <- as.numeric(format(as.Date(bongo2020_taxa_meta$eventDate), "%m"))
bongo2020_taxa_meta$day <- as.numeric(format(as.Date(bongo2020_taxa_meta$eventDate), "%d"))
```
...
Lets first create an Occurrence Core for the (estimated) abundance data. The abundance data file also includes data from 2019 so we first need to filter out only the data collected during the 2020 High Seas Expedition. 

``` {r bongo2020_taxa_occ, eval = FALSE}
bongo2020_abun <- bongo2020_taxa_meta %>%
  mutate(# Confirm that this study is only done using samples collected in Net1. 
         cast = paste(station, "1", sep = ":bongo:Net"))
bongo2020_abun$date <- as.Date(bongo2020_abun$date) # somehow need to include this line because otherwise in the code line below it doesn't see 2020-03-12 as date. 
bongo2020_abun <- subset(bongo2020_abun, date >= "2020-03-12" & date <= "2020-04-04")
...
bongo2020_occ$Species <- gsub("\\(|)", "", bongo2020_occ$Species) 
bongo2020_occ$Species <- gsub(c("Gelatinous spp.|Jelly unknown"), "Doliolida spp.", bongo2020_occ$Species)
...
# As you can see there are still rows where no taxonomic data is provided for the scientificName. This is for data entries such as "Squid" and "Fish eggs". For data entries such as "Gelatinous spp." and "Jelly (unknown)" I confirmed with the data provider that these are likely Doliodid spp. (best guess). I have attempted to reflect this best as possible in the dwc terms. Please note that ( ) are special characters for R and so these need to be replaced first. 
...
bongo2020_occ_fnl <- bongo2020_occ_fnl %>%
  mutate(verbatimTaxonRank = ifelse(grepl("sp.",bongo2020_occ_fnl$Species), "sp.", NA),
         verbatimTaxonRank = ifelse(grepl("spp.",bongo2020_occ_fnl$Species), "spp.", NA))
...
# Make sure the folder path exists already (e.g. ./Bongo/tidy_data)
write_csv(bongo_emof_abun, here("Bongo", "tidy_data", "bongo2020_emof_abun.csv"))
```

Finalize the entire extended Measurement or Fact core for the 2020 Bongo Zooplankton Data. 

``` {r bongo_measurementOrFact Core, eval = FALSE}
bongo2020_measurementOrFact <- bind_rows(bongo2020_bottomdepth, bongo2020_sampling, 
                                     bongo2020_instrument, bongo2020_emof, bongo2020_emof_abun) %>%
