...
# Save the occurrence Core locally and in Google Drive:
write_csv(samplingInstrument, here("Trawl", "tidy_data", "trawl2020_samplingInstrument_emof.csv"))
```
...
count - total: http://vocab.nerc.ac.uk/collection/P01/current/OCOUNT01/
...
trawl2020_allcatch_measurement <- trawl2020_occ %>%
  select(occurrenceID,
         ...
         `CATCH_WEIGHT (KG)`) %>%
  mutate_all(as.character) %>%
  pivot_longer(cols = c(organismQuantity, `CATCH_WEIGHT (KG)`),
               names_to = "measurementType",
               values_to = "measurementValue")

trawl2020_methods <- trawl2020_occ %>%
  select(occurrenceID,
         `COUNT_METHOD                 (Total, Volume est, Weight est)`,
         WEIGHT_METHOD) %>%
  mutate_all(as.character) %>%
  pivot_longer(cols = c(`COUNT_METHOD                 (Total, Volume est, Weight est)`, WEIGHT_METHOD),
                        values_to = "measurementMethod") %>%
  select(measurementMethod)

trawl2020_allcatch_emof <- cbind(trawl2020_allcatch_measurement, trawl2020_methods) %>% 
  mutate(measurementType = recode(measurementType,
                                  `CATCH_WEIGHT (KG)` = "Total species biomass",
                                  organismQuantity = "count"),
         measurementID = case_when(
           measurementType == "count" ~ paste(occurrenceID, "count", sep = "-"),
         ...
         measurementTypeID = case_when(
           ...
           measurementType == "count" ~ "http://vocab.nerc.ac.uk/collection/P09/current/OCNT/"),
         # When possible there might be new terms created for counts done through visual estimation etc. 
         measurementUnit = case_when(
