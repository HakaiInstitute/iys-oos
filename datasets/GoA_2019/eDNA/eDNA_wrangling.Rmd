...
Read in the dataset from your local drive, and combine the data into one dataframe. The data file `2020_2019_sample_info.xlsx` contains the information as to which station the eDNA was collected at. However, no date, time or coordinates are included, these will likely have to come from the Bridgelog file that we have yet to receive for 2019. Once received, information from that Bridgelog file will be included in the metadata associated to the Event Core of eDNA standardization. 

Let's first sub-set for the eDNA data collected during the 2020 cruise, as we do have the Bridgelog sheet from that expedition, but not from the 2019 expedition. From the eDNA_sample_info datasheet, we can determine that these have been classified under Cruise_program 'IYS_GoA_2020'. Using these unique samples for 2020, subset the relevant columns in the detections datasheet, so that only those relevant to 2020 are selected. 

How to deal with the control (H20) samples in the dataset?
...
# The following lines are to select the columns from the dataset specific to the 2020 Expedition, by linking it to the unique samples from eDNA_sample_info dataframe. 
...
# The Event Core will have four 'layers' to which specific metadata will get associated: 'project', 'cruise', 'station' and 'sample':
...
# Join date and coordinates to the eDNA sampling from the bridgelog data sheet. No precise coordinates related to the eDNA sampling are collected, so the trawl coordinates are used.
...
eDNA_sample <- left_join(eDNA, bridgelog, by = "Set") %>%
  ...
  filter(!is.na(END_LAT_DECDEGREE)) 
  # Station 20 needs end lat and long included - this issue needs to be resolved. 
...
eDNA_sample <- cbind(eDNA_sample, eDNA_coordinates) %>%
  ...
  mutate(minimumDepthInMeters = " ",
         ...
         year = as.numeric(format(as.Date(eDNA_sample$eventDate), "%Y")),
         month = as.numeric(format(as.Date(eDNA_sample$eventDate), "%m")),
         day = as.numeric(format(as.Date(eDNA_sample$eventDate), "%d")))
...
drive_upload(here("eDNA", "tidy_data", "edna_event.csv"),
             path = "https://drive.google.com/drive/u/0/folders/1uofTO34XpWiSHf7WzSdefY83uyrwjie2",
             name = "edna_event.csv",
             overwrite = TRUE)
```
...
In the dataset `eDNA_detections`, the column OTU (Operational Taxonomic Units) lists all the species (taxa) observed or tested for. The values in the columns indicate the reads in the library assigned to the appropriate OTU. Reads >= 10 indicates a positive detection (`occurrenceStatus` = present), whereas reads = 0 indicates absence (`occurrenceStatus` = absent). However, data users will have to determine whether this constitutes true species' absence, based on target gene used. 
...
# Connect the dataframe to station number (Set) from the eDNA_sample_info dataframe. The strings in the 'strings' column only partially match the string under 'sample' in the eDNA_sample_info dataframe, so use the fuzzyjoin package. We connect these two dataframes because at later stage we want to use that information to construct the unique occurrenceIDs. 
...
# Upon manual inspection, there are 29 "taxa" for which no record exists in the WoRMS registry, or whose observation will have to be confirmed with the data provider. Once this is done the information will either have to be entered manually or if new records created in WoRMS, the code can be run again. 
