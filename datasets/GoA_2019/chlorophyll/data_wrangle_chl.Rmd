...
chl <- read_excel(here("chlorophyll", "raw_data", 
  ...
  mutate(Time = format(Time, format = "%H:%M:%S"),
         ...
         ndepth = paste(cast, Depth, sep=":niskin:"),
...
The "station" event category exists to tie casts and trawl data together. eventDate, decimalLatitude and decimalLongitude are attributes associated with the "cast" record type, because sampling times and cast and trawl coordinates differ among datasets. I think `footprintWKT`, though useful to calculate the `coordinateUncertaintyInMeters`, is not really required when it concerns accurate point locations.  

Finally, these tables get joined, and if required can be re-ordered based on `eventID`. Additionally, `geodeticDatum` gets added. This table then gets saved locally and in the correct GoogleDrive folder. 
...
chl_cast <- chl %>%
  ...
  mutate(eventRemarks = "cast",
         footprintWKT = paste("POINT", " (", chl_cast$decimalLongitude,
                              " ", chl_cast$decimalLatitude, ")"))

coordinates <- obistools::calculate_centroid(chl_cast$footprintWKT) %>% select(coordinateUncertaintyInMeters)
chl_cast <- cbind(chl_cast, coordinates)
...
chl_event <- bind_rows(chl_cruise, chl_station, chl_cast, chl_ndepth, chl_sample) %>% select(eventID, parentEventID:maximumDepthInMetres, coordinateUncertaintyInMeters, eventRemarks) %>% select(-footprintWKT) %>%
  mutate(type = "Event",
...
obistools::check_eventids(chl_event)

# Should there be a need to recursively add event information from parent events to 'child events', use:
# obistools::flatten_event(chl_event)
...
chl_measurement <- chl %>%
  ...
  mutate(measurementID = case_when(measurementType == "Chl" ~ paste(eventID,"chl", sep=":"),
         ...
         measurementTypeID = recode(measurementType, #URIs from NERC website
                                    "chlorophyll a" = "http://vocab.nerc.ac.uk/collection/P01/current/CPHLHPP1/",
                                    "phaeopigments" = "http://vocab.nerc.ac.uk/collection/P01/current/PHAEZZXX/"),
  ...
  dplyr::rename(decimalLatitude = Latitude,
                decimalLongitude = Longitude,
                eventDate = dateTime) %>%
  select(eventID, decimalLatitude, decimalLongitude, eventDate,
         measurementID, measurementType, measurementTypeID,
...
chl_leaflet <- obistools::plot_map_leaflet(chl_event)
chl_map <- obistools::plot_map(chl_event, zoom = TRUE)
ggsave(filename = "chl_map.png", plot = chl_map, path = here::here("chlorophyll", "maps"))
```

ERDDAP is used as a repository to host physical and geochemical data, among other things. Therefore, we want to have a separate output for ERDDAP, where the data is structured and reformatted in a way that flows well with ERDDAP. ERDDAP works with a wide format, rather than a long format (OBIS). The column is renamed to standard convention following the CF Conventions (https://cfconventions.org/Data/cf-standard-names/75/build/cf-standard-name-table.html).

**However, for phaeopigment concentration I have not yet been able to find standardized vocabulary in CF Conventions - I have a proposed a standard name through the repository, see: https://github.com/cf-convention/discuss/issues/81**. 

``` {r erddap, eval = FALSE}
chl_erddap <- chl_measurement %>% select(eventID, decimalLatitude,
                                         decimalLongitude, eventDate,
                                         measurementType, measurementValue, measurementUnit)
chl_erddap <- pivot_wider(chl_erddap, names_from = measurementType,
                          values_from = measurementValue) %>%
  rename(Chl = `chlorophyll a`,
         Phe = phaeopigments) %>%
  mutate(mass_concentration_of_chlorophyll_a_in_sea_water = Chl,
         phaeopigment_concentration = Phe) %>%
  select(eventID, decimalLatitude, decimalLongitude, eventDate, mass_concentration_of_chlorophyll_a_in_sea_water,
         phaeopigment_concentration)

chl_new <- chl %>% select(Depth, sample) %>%
  rename(eventID = sample)

chl_erddap <- left_join(chl_erddap, chl_new, by = "eventID")
write_csv(chl_erddap, here("chlorophyll", "tidy_data", "chl_erddap.csv"))
```
