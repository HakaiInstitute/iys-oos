...
After creating the Event Core, we create an extended MeasurementOrFact (eMoF) table. An occurrence table is not created as the datasets only contain abiotic data. In the wrangling for the measurement table, we create specific eventIDs that we can associate measurement values to at a later stage. Also, DwC-A standards work with "long" format, rather than the wide format, so we need to transpose the data. 

``` {r measurement wrangle, eval = FALSE}
...
__Note: in the previous chunk we got our data from sheet1 - this is data collected at discrete depths. To create a measurement table for sheet2, simply replace sheet1 with sheet2.__

Assign the correct NERC controlled vocabulary to the variables and the measurement units. 
...
Finally, the eMoF table gets saved locally and on GoogleDrive. 

``` {r CTD save, eval = FALSE}
