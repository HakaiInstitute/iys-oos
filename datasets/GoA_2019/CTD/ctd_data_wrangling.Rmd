...
ctd_event <- bind_rows(ctd_cruise, ctd_station, ctd_cast, ctd_ndepth, ctd_sample) %>% select(eventID, parentEventID:maximumDepthInMetres, coordinateUncertaintyInMeters, eventRemarks) %>% select(-footprintWKT) %>%
  mutate(type = "Event",
