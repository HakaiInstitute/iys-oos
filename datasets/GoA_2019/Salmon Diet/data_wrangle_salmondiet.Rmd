...
predator$id <- " "
predator <- predator %>% 
  select(eventID, species, n_stomachs) %>% distinct()

for(i in unique(predator$eventID)) {
  predator$id[predator$eventID == i] <- seq_len(sum(predator$eventID == i))
}

predator$predator_id <- paste0("p", predator$id)
predator$eventID <- paste(predator$eventID, predator$predator_id, sep = ":")
...
spp_occ <- spp_occ[!is.na(spp_occ$presence), ] %>% distinct()
...
salmon_sample <- salmon_sample %>%
  mutate(footprintWKT = " ")
salmon_sample$footprintWKT <- paste0("POINT (", salmon_sample$decimalLongitude, " ",
                                     salmon_sample$decimalLatitude, ")")
...
predator$occurrenceID <- predatorpaste(predator$eventID, predator$predator_id, sep = ":")

predator_occ <- predator %>%
  dplyr::select(eventID, occurrenceID, species,
                n_stomachs, predator_id) %>%
...
diet_data_occ <- bind_rows(salmon_prey_occ, predator_occ)
