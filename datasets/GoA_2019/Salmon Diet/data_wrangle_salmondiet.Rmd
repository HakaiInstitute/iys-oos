...
Now read both files from the local folder, and make the minor adjustments needed to cross-reference columns `Trawl`, `Species` and `Mass`. 
...
test <- inner_join(salmon, pathogen) %>% mutate(check = "match")
# The rows where there is no match can be found using:
test2 <- anti_join(salmon, pathogen) %>% mutate(check = "nomatch")
full <- rbind(test, test2)
full <- full[order(full$Trawl),]
```

Using the nrow() function we find that there are 52 rows where `Species`, `Trawl` and `Mass` data matches between the salmon diet data and the pathogen data, and 96 rows where there is no match. Unfortunately this means that we cannot cross-reference the pathogen and salmon data for the majority of the fish, and consequently that - at the moment - we are not able to use Floytag or MGL# as a common parentEvent. 
...
Please note that the date and coordinates - given that it's currently from a .xlsx file that is not considered the final datasheet - could change. Will need to make sure that the correct excel file is read. 
...
# Once information on minimumDepthInMetres and maximumDepthInMetres regarding the trawl becomes available we need to either add that to salmon_sample or salmon_trawl. If we opt for two occurrence cores that are connected to the eventID through a common occurrenceID, then the following lines of code are not necessary: 
...
salmon_spp_measurement <- salmon %>%
  ...
  mutate(measurementID = case_when(
    ...
    measurementType == "mass_g" ~ " ",
    measurementType == "food_mass_g" ~ " ",
...
Q: Similar to the question above regarding the occurrence cores, should these measurement or fact cores be combined into a single MeasurentOrFact Core? 
