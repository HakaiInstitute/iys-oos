...
unique_salmon <- unique(pathogen_salmon_spp$Species)

pathogen_salmon_worms <- taxizesoap::worms_records(unique_salmon) %>% filter(inputid == scientificname) %>%
  rename(Species = inputid)

pathogen_salmon_spp <- left_join(pathogen_salmon_spp, pathogen_salmon_worms, by = "Species") %>%
  ...
  select(eventID, Sample, occurrenceID, scientificName, scientificNameID, occurrenceStatus,
...
pathogen_taxa <- obistools::match_taxa(unique_pathogens) 
pathogen_taxa_worms <- cbind(unique_pathogens, pathogen_taxa) %>% filter(!is.na(scientificName))
pathogen_taxa_valid <- taxizesoap::worms_records(pathogen_taxa_worms$scientificName) %>% 
  filter(inputid == scientificname) %>%
  select(valid_name)
pathogen_taxa_full <- taxizesoap::worms_records(pathogen_taxa_valid$valid_name) %>% filter(inputid == scientificname) %>%
  rename(scientificName = scientificname,
         taxonRank = rank,
         scientificNameID = lsid) %>%
  select(scientificName, scientificNameID, taxonRank, kingdom, phylum, class, order, family, genus)

# Any species that need to be entered manually? 
pathogen_manual <- cbind(unique_pathogens, pathogen_taxa) %>% filter(is.na(scientificName))

# We notice that for two entries the valid or accepted name in WoRMS is slightly different from that recorded in the data, and found in the pathogen_occ dataframe. In order to match the taxonomic information in pathogen_taxa_full, the scientific names in this dataframe need to be substituted with the correct or accepted name:

pathogen_occ <- pathogen_occ %>%
  mutate(Name = gsub("Vibrio anguillarum", "Listonella anguillara", Name),
         Name = gsub("Vibrio salmonicida", "Aliivibrio salmonicida", Name),
         Name = gsub(" sp.", "", Name)) %>%
  rename(scientificName = Name)

pathogen_occ_fnl <- left_join(pathogen_occ, pathogen_taxa_full, by = "scientificName") %>%
  rename(eventID = trawl) %>%
  mutate(occurrenceID = paste(eventID, Sample, sep = ":"),
         occurrenceID = paste(occurrenceID, "p", sep = ":"), # to indicate it concerns a pathogen
         occurrenceID = paste(occurrenceID, `Assay name`, sep = ":"),
         occurrenceStatus = "present") %>%
  select(eventID, Sample, occurrenceID, occurrenceStatus, scientificName, scientificNameID, taxonRank,
...
pathogen_final_occ <- rbind(pathogen_salmon_spp, pathogen_occ_fnl)

order <- stringr::str_sort(pathogen_final_occ$occurrenceID)
pathogen_final_occ <- pathogen_final_occ[match(order, pathogen_final_occ$occurrenceID),]

pathogen_final_occ <- pathogen_final_occ %>%
  mutate(associatedTaxa = ifelse(grepl(":p:", pathogen_final_occ$occurrenceID),
                                 paste("pathogen found on: ", pathogen_final_occ$eventID, Sample, sep = ""), ""))
```
...
# Make sure the folder path exists already (e.g. ./Salmon Diet/tidy_data)
write_csv(pathogen_final_occ, here("Pathogen", "tidy_data", "pathogen_occ.csv"))
...
Next we need to associate the values reported (relative Ct-values) to the various pathogens.  

``` {r eMOF, eval = FALSE}
pathogen_spp_measurement <- pathogen %>%
  select(sample, 
         ae_sal:ye_ruc_glnA) %>%
  rename(measurementID = sample) %>%
  pivot_longer(cols = ae_sal:ye_ruc_glnA,
               names_to = "measurementType",
               values_to = "measurementValue") %>%
  filter(measurementValue > 0) %>%
  mutate_all(as.character) %>%
  mutate(measurementID = paste(measurementID, "p", sep = ":"),
         measurementID = paste(measurementID, measurementType, sep = ":"),
         measurementID = paste(measurementID, "Rel.Ct-value", sep = ":"))
