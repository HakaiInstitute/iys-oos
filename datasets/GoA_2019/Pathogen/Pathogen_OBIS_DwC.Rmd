...
drive_download(file = "https://docs.google.com/spreadsheets/d/1Teh8Lcj3vCxocB2HPso19XYrZvCUspFt/edit#gid=1038314498", 
               path = here::here("Pathogen", "raw_data", "2019_GoA_Fish_Trawl_catchdata.xlsx"),
               overwrite = TRUE)

trawl <- read_excel(here("Trawl", "2019", "raw_data", "2019_GoA_Fish_Trawl_catchdata.xlsx"), sheet = "BRIDGE_LOG_FINAL")
...
# Somehow the stations are all shuffled, re-order them:
...
pathogen_salmon_spp <- left_join(pathogen_salmon_spp, pathogen_salmon_scientificnames, by = "scientificName") %>%
  ...
  mutate(specificEpithet = stringr::word(pathogen_salmon_spp$scientificName, 2))
...
pathogen_taxa <- left_join(pathogen_occ, pathogen_taxa_scientificnames, by = "scientificName") %>%
  ...
  mutate(specificEpithet = NA)
```

Perhaps we still have to wait for is new additions being made in the WoRMS registry to include the parasites, viruses or bacteria. Once this is done, we need to combine the occurrence tables of salmonids and pathogens. In addition to this, we have to indicate that the pathogens were found on the salmonids. To do this, we do two things (for now): 1) create an `resourceRelationship` extension and 2) include columns `associatedTaxa` and `associatedOccurrence` in the Occurrence extension to indicate that the pathogens were found on or in the salmonid individual (i.e. linking it to the occurrenceID and to the salmon individual): 
...
The resourceRelationship extension, saved locally:

``` {r resourceRelationship, eval = FALSE}
pathogen_resourceRelationship <- pathogen_occ %>%
  select(occurrenceID, temp_Sample) %>%
  mutate(relatedResourceID = ifelse(grepl(":p:", pathogen_occ$occurrenceID), pathogen_occ$occurrenceID, NA),
         resourceID = temp_Sample,
         relationshipOfResource = ifelse(!is.na(relatedResourceID), "host of", NA)) %>%
  drop_na() %>%
...
write_csv(pathogen_resourceRelationship, here("Pathogen", "tidy_data", "pathogen_resourceRelationship.csv"))
```

And include the columns `associatedTaxa` and `associatedOccurrence` to the Occurrence extension: 

``` {r pathogen_associatedTaxa, eval = FALSE}
pathogen_occ <- pathogen_occ %>%
