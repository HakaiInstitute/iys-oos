...
bongo$sample_id <- gsub("\\b.0\\b", "", bongo$sample_id)
bongo <- bongo %>%
  mutate(project = "IYS",
...
bongo_station <- bongo_station %>%
  mutate(eventRemarks = ifelse(grepl("IYS:GoA2019_Stn51", bongo_station$eventID), "Bongo cancelled because of weather", 
                               ifelse(grepl("IYS:GoA2019_Stn52", bongo_station$eventID), "Bongo cancelled because of weather",
                                      NA)))
...
bongo_event <- bind_rows(bongo_project, bongo_cruise, bongo_station, bongo_cast, bongo_sample) %>% 
  mutate(basisOfRecord = "HumanObservation", 
         #should any sample from Net2 be PreservedSpecimen, and samples from Net 1 be "Human Observation"?
...
An `occurrenceID` and `occurrenceStatus` will only be provided to data entries where species have been assigned a WoRM URN. Entries where Species = `Size fraction`, will not have an occurrenceID associated to it, because it is not possible to associate a URN from WoRMS to this class or group (`Zooplankton`). The same goes for entries where no common rank can be associated, such as `Jellyfish, radiolarian, chaetognath` and `Gelatinous`. So first we inspect all the unique input for the `species` column, and filter out the species that can _not_ be identified. Measurements of weight, length, and life stage will still be included, however they will linked directly to the `eventID` (the sample_id) rather than the `occurrenceID`. In other words, these data entries will not be included in the Occurrence extension!
...
bongo2019_taxa_abundance <- bongo2019_abun %>% select(-c(ind_total, abund_m3_flow_est, abund_m3_depth_est))
# Make sure the folder path exists already (e.g. ./Bongo/tidy_data)
...
So now we've created the first Occurrence table coming from the Bongo net 1 zooplankton data, on community analysis / abundance estimates. These `occurrenceID`s will have associated measurements linked to it as well, which should include `sex`, `lifeStage`, and the various ways in which abundance or total number is estimated? i.e. `ind_total` = total number of individuals estimated in the entire sample, `abund_m3_flow_est` = total abundance estimated from the flow meter, and `abund_m3_depth_est` = total abundance of individuals estimated using the depth-estimated estimate. These terms might require new additions to controlled vocabulary, or alternatively, these get included as _measurementMethod_. 

*** 

The second Occurrence table we create is on the data collected for the dry weight isotope and fatty acid analysis. This data is found in a different data set (`bongo`). Some larger individuals from Net 1 were used both in the abundance study as well as fatty acid/isotope study. 
...
Length and weight measurements associated with unidentified species will eventually also be added to the eMOF extension, however these will be linked to the `eventID`s. As such, we have to manually change records where length is reported in a range (e.g. 16-20mm) or in an approximation (>4mm). Additionally, we add `occurrenceRemarks` to the records where number is reported as ~3 or >3, to indicate the uncertainty in count. 

The following chunk of code will eventually be used to create the measurementOrFacts related to unidentified species.

``` {r intermezzo, eval = FALSE}
unidentified_spp$minimumLength <- NA
unidentified_spp$maximumLength <- NA
unidentified_spp$number <- as.character(unidentified_spp$number)
unidentified_spp$minimumLength <- ifelse(grepl(">4mm", unidentified_spp$length_mm), "4", unidentified_spp$minimumLength)
unidentified_spp$minimumLength <- ifelse(grepl("16-20mm", unidentified_spp$length_mm), "16", unidentified_spp$minimumLength)
unidentified_spp$maximumLength <- ifelse(grepl("16-20mm", unidentified_spp$length_mm), "20", unidentified_spp$maximumLength)
unidentified_spp$measurementRemarks <- ifelse(grepl(">4mm", unidentified_spp$length_mm), 
                                             "Count represents a minimum recorded value", NA)
unidentified_spp$length_mm <- ifelse(grepl(">4mm", unidentified_spp$length_mm), NA, unidentified_spp$length_mm) 
unidentified_spp$length_mm <- ifelse(grepl("16-20mm", unidentified_spp$length_mm), NA, unidentified_spp$length_mm)

unidentified_spp$measurementRemarks <- ifelse(grepl(">3", unidentified_spp$number),
                                              "Count represents a minimum recorded value",
                                              unidentified_spp$measurementRemarks)
unidentified_spp$measurementRemarks <- ifelse(grepl("~3", unidentified_spp$number),
                                              "Count represents approximate value", 
                                              unidentified_spp$measurementRemarks)
unidentified_spp$number <- ifelse(grepl(">3|~3", unidentified_spp$number), "3", unidentified_spp$number)
```

The following code creates the occurrence table from species observed (and assigned a WoRMS URN):
...
Finally, the dataframe of these two sample_ids will be joined with the `bongo2019_taxa_occ` dataframe. 
...
**To address these issues, a total of five different occurrence tables will be created, that eventually get merged to a single Occurrence extension! Reminder: These are species observations from Net 2, and in all these occurrence tables only data entries are included where species have been identified to species or genus level (so have an associated WoRMS URN), and will therefore have an associated occurrenceID and occurrenceStatus.**
...
bongo2019_occ1$number <- gsub("[>]", "", bongo2019_occ1$number)
bongo2019_occ1$occurrenceRemarks <- "Count represents a minimum number observed"
```
...
To create the final occurrence extension, the five occurrence tables are joined. As individual measurements are associated to these data entries, an `occurrenceID` is created for them. 
...
We first create the eMoF table for the samples collected _mostly_ from Net 2 (for the fatty acid/isotope analysis) that links directly to the `eventID`. This is done to those species for which no WoRMS URN could be assigned (i.e. the unidentified species):
...
bongo_emof <- unidentified_spp %>%
  select(eventID = sample, 
         ...
         maximumLength,
         weight = weight_mg,
         length_type, measurementRemarks) %>%
  mutate_all(as.character) %>%
  ...
  mutate(measurementID = case_when(
      ...
      measurementType == "individualCount" ~ paste(eventID, "count", sep = ":"),
...
When the species or genus has been identified, the measurements are associated to the `occurrenceID`. This will be included in a second eMOF extension (emof2):  
...
bongo_emof2$measurementRemarks <- ifelse(grepl(">", bongo_emof2$individualCount), "Count value represents a minimum", NA)
bongo_emof2$individualCount <- gsub(">", "", bongo_emof2$individualCount)

# Additionally, there are reported values under "length" that indicate a minimum length (i.e. >4mm):
bongo_emof2$minimumLength <- ifelse(grepl(">4mm", bongo_emof2$length), "4", bongo_emof2$minimumLength)
bongo_emof2$minimumLength <- ifelse(grepl(">2mm", bongo_emof2$length), "2", bongo_emof2$minimumLength)
bongo_emof2$length <- gsub(">", NA, bongo_emof2$length)
```
...
# There are fields under measurementValue that are "NA" or NA. These need to be removed from the eMOF extension.
bongo_emof2 <- bongo_emof2 %>% filter(measurementValue != "NA") 
```
...
bongo2019_net2_emof <- bind_rows(bongo_emof, bongo_emof2)  
bongo2019_net2_emof <- merge(bongo2019_net2_emof, bongo_measurementRemarks,
                      by = c("eventID", "measurementRemarks"), all = T)

write_csv(bongo2019_net2_emof, here("Bongo", "tidy_data", "bongo2019_emof_Net2.csv"))
```

The second eMOF extension we create is for the abundance / community analysis study, which is from the `bongo2019_abun` dataframe. Need to figure out whether to use the individualCount or the total count. Additionally, perhaps associated to an abundance estimate we need to assign whether this was estimated by flowmeter or depth-estimate, under _measurementMethod_. 

- [ ] TO DO: Add _measurementMethod_ to abundances estimated by those methods. 
...
# After manual inspection of the bongo_measurementOrFact extension: 
# remove rows where measurementValue is NA or NA: 
bongo_measurementOrFact <- bongo_measurementOrFact[!(bongo_measurementOrFact$measurementValue == "NA") | 
                                                   !(bongo_measurementOrFact$measurementValue == "ND"),]
