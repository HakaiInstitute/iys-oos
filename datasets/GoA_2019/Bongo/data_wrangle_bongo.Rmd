...
bongo_fnl <- bongo_metadata <- read_excel(here("Bongo", "raw_data",
                                  "IYS cruise logbook_2019.xlsx"),
...
bongo_station <- bongo_fnl %>%
  ...
  mutate(notes = case_when(
    eventID == "GoA2019_Stn51" ~ "Bongo cancelled because of weather",
    eventID == "GoA2019_Stn52" ~ "Bongo cancelled because of weather"
  ))
...
# To recursively add event information from parent events to child events:
...
Species <- unique(bongo_occ_spp$species)
scientificNames <- obistools::match_taxa(Species)
scientificNames <- cbind(Species, scientificNames)

names_worms <- taxizesoap::worms_records(scientificNames$scientificName) %>%
  select(inputid, scientificname, authority, rank, valid_name, valid_authority, kingdom,
         phylum, class, order, family, genus, lsid)
names_worms <- names_worms %>% rename(scientificName = scientificname)

x <- left_join(scientificNames, names_worms, by = "scientificName") %>% select(Species, valid_name)
```
