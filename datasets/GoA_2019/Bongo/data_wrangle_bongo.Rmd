...
An `occurrenceID` and `occurrenceStatus` will only be provided to data entries where species have been identified to a species or genus level. Only for these species can a URN be associated from WoRMS. So first we inspect all the unique input for the `species` column, and filter for all these species by filtering out the species that can _not_ be identified to a species or genus level. Measurements of weight, length, life stage and taxonomic group will still be included, however they will linked directly to the `eventID` (the sampleID) rather than the `occurrenceID`. In other words, _no_ Occurrence Core will be created for these data entries!
...
# Upon inspection, numerous bongo tows have the sample_id 'Petri dish' associated to them, with the only distinguishable feature being a difference in length measurement. In our current set up, all Petri dish sample_ids belong to the same Station have the same sample (GoA2019_Stn10:cast1:bongo:Net1:Petri dish), although they are different data entries. Therefore we attach a unique sample to them and then merge them back to the dataframe: 
bongo_occ_spp_petri <- bongo_occ_spp %>% filter(sample_id == "Petri dish") %>%
  mutate(sample_id = paste(sample_id, row_number(), sep = "_"))
bongo_occ_spp_2 <- bongo_occ_spp %>% filter(!sample_id == "Petri dish")
bongo_occ_spp <- rbind(bongo_occ_spp_2, bongo_occ_spp_petri)
```

Upon close inspection of the data that we filtered out, there are two sample_ids (SS81 and SS133) for which the species are mentioned in the _notes_ column, _along_ with their length measured. For these two sample_ids we can add an `occurrenceID` and `occurrenceStatus`. Both sample_ids require special attention: For sampleID SS81 the number = 7, but the notes distinguish 8 unique measurements. Additionally, sample_id SS133 requires special attention because although number = 1, in the notes it says that jellies were observed (length range: 10 - 15mm) and 3 sagitta spp. were observed, with individual lengths measured. 

We will split up the data entries for these two sample_id's, thereby creating unique entries, and join it back with the dataset `bongo_occ_spp`. To create unique rows for these data entries, we manually change the value in the `number` column.
...
# Then we duplicate the row of the sampleID based on the value in the number column. The new dataframe consists only of duplicated rows, and will later be merged again with the dataset containing the original two sampleIDs/rows. The duplicated rows therefore need to reflect that no unique weight measurements are associated. 
bongo_pre_wrangle_dup <- data.frame(bongo_pre_wrangle[rep(seq_len(dim(bongo_5)[1]),
...
* Q: If a data entry has number > 1, but only one associated length measurement, does that mean that all individuals in that row have that length? If so - these should all receive a unique `occurrenceID` and `occurrenceStatus`!

__To address these issues, a total of five different occurrence tables will be created, that eventually get merged to a single Occurrence Core! Reminder: in all these occurrence tables only data entries are included whether species have been identified to species or genus level, and will therefore have an associated occurrenceID and occurrenceStatus.__
...
When filtering for number > 1, we noticed that there are four data entries (rows) with a similar sample_id ("Petri dish"). In other words, for sample_id `Petri dish` there are four data entries, all for the same species, so in our current set up these all have the same `sample_id` (GoA2019_Stn11:cast1:bongo:Net1:Petri dish). We will deal with these entries separate. So our first step here is to filter out this sample_id and assign a unique `eventID`. However, upon inspection, unique length measurements are provided in the notes for only three of these data entries.

Therefore we have to deal with sample_id 'Petri dish' by creating unique `eventID`s, and then splitting three of those uniquely created `eventID`s as they have individual measurements associated to them. The fourth uniquely created `eventID` will have number = 3, but only a single length measurement associated. Therefore this row will be removed and be dealt with 
...
The next step is to create duplicate rows of these data entries, equal to the count in the `number` column. These duplicated rows will need to reflect the individual length (mm) measurements and also reflect that it now represents just a single measurement (i.e., number = 1). Additionally, for all the split unique data entries in this dataframe, the weight_mg measurement has to be NA. The weight_mg measurements for these data entries will still be reflected in the original data. In a later stage, this newly created dataframe will be merged with the original dataframe, and the `occurrenceIDs` will be given a unique string. 
...
# The resulting bongo_occ5 dataframe includes data entries where the number > 1, but no individual length measurements or length range is provided. However, all these data entries have been identified to a species or genus level. 
...
After an initial QC of the Occurrence Core .csv file, Tim noticed that two sample_ids have 2 `OccurrenceID`s associated to it whereas it should only be 1. The sample_ids are: Z372 and and 22003. -- This issue has been resolved.
...
In our extended measurementOrFact table(s), we associate the measurements to either the `eventID` (if species was not identified to species or genus level), or the `occurrenceID` (if species was identified to species or genus level). As a first step though, we associate the _depth_ to the cast. 
...
bongo_bottomdepth <- bongo %>% 
  ...
  mutate(measurementTypeID = case_when(
    ...
    measurementType == "seafloor depth" ~ "http://vocab.nerc.ac.uk/collection/P06/current/ULAA/")) %>%
...
Not sure if this means that we have to create two distinct extended measurementOrFact tables which we join to a single eMoF Core? One linking to the `occurrenceIDs` and the other directly to the `eventID`. We first create the eMoF table that links directly to the `eventID`. First we have to filter for all the data entries for which no `occurrenceID` has been created. In other words, remove any rows (data entries) for which an `occurrenceID` has been created:
...
bongo_emof <- bongo_emof %>%
  ...
  mutate(
    measurementID = case_when(
      ...
      measurementType == "group" ~ paste(eventID, "group", sep = ":")
    ),
    measurementTypeID = case_when(
      ...
      measurementType == "group" ~ " "
    ), 
    measurementUnit = case_when(
      measurementType == "length" ~ "mm",
      measurementType == "weight" ~ "g",
      measurementType == "size_fraction" ~ "microns" 
...
As there were also comments in the dataset, we have to make sure we include those in the measurementOrFact table, as they might be important to the measurements. 
