...
Two Occurrence tables will be created, as we're dealing with two separate datasets created from the same data collection event. One dataset includes dry weight and length measurements, whereas the other dataset contains species-specific (estimated) abundance at each sampling station, with sex and lifestage. From my understanding, samples in Net 1 from the bongo sampling instrument were typically fixed as a whole for community and abundance analysis. Zooplankton in Net 2 was used predominantly for isotope or fatty acid analysis. 
...
After internal discussion, it was opted to associate an `occurrenceID` to an entry as long as we can find a WoRMS taxon / URN, even if a species can only be identified up to a phylum or kingdom level. Entries where Species = `Size fraction`, will not have an occurrenceID associated to it, because it is not possible to associate a URN from WoRMS to this class or group (`Zooplankton`). The same goes for entries where no common rank can be associated, such as `Jellyfish, radiolarian, chaetognath` and `Gelatinous`. 

**However, a new term might be introduced to the Occurrence extension: `verbatimScientificName` (see [here](https://github.com/tdwg/dwc/issues/181)), in which case these data entries might be included under this column.**
...
So now we've created the first Occurrence extension coming from the Zooplankton data, on community analysis / abundance estimates. This Occurrence extension will need to have an associated eMOF extension linked to it as well, which should include `sex`, `lifeStage`, and the various ways in which abundance or total number is estimated? i.e. `ind_total` = total number of individuals estimated in the entire sample, `abund_m3_flow_est` = total abundance estimates from the flow meter, and `abund_m3_depth_est` = total abundance of individuals estimated using the depth-estimated estimate. These terms might require new additions to controlled vocabulary. 

*** 

The second Occurrence extension we create is on the data collected for the dry weight isotope and fatty acid analysis. This data is found in a different data set (`bongo`). Though some larger individuals from Net 1 were used both in the abundance study as well as fatty acid/isotope study, these are _not_ included in the standardized data. The reason for this is that there is no way for us to determine which samples were removed from Net 1. The only information listed is whether or not the species came from Net 1.  

**Important data limitation alert**: Some individuals from Net 1 were also used in the fatty acid / isotope analysis. These individuals have been **filtered out** of the data because we could not trace back exactly which individuals were used in both studies. While this has no consequences for presence/absence records in OBIS, the length/weight measurements associated to these records are not included in the standardized data.
...
**To address these issues, a total of five different occurrence tables will be created, that eventually get merged to a single Occurrence extension! Reminder: in all these occurrence tables only data entries are included where species have been identified to species or genus level (so have an associated WoRMS URN), and will therefore have an associated occurrenceID and occurrenceStatus.**
...
We first create the eMoF table that links directly to the `eventID`. First we have to filter for all the data entries for which no `occurrenceID` has been created. In other words, the dataframe of unidentified_spp:
...
bongo_emof2 <- bongo_emof2 %>% filter(measurementValue != "NA") %>% drop_na(measurementValue)
```
...
bongo_study1 <- bind_rows(bongo_emof, bongo_emof2)  
bongo_study1 <- merge(bongo_study1, bongo_measurementRemarks,
                      by = c("eventID", "measurementRemarks"), all = T)

write_csv(bongo_study1, here("Bongo", "tidy_data", "bongo2019_emof_study1.csv"))
```

The second eMOF extension we create is for the abundance / community analysis study, which is from the bongo2019_abun dataframe. Need to figure out whether individualCount and the total individualCount. 
...
# Make sure the folder path exists already (e.g. ./Bongo/tidy_data)
write_csv(bongo_emof_abun, here("Bongo", "tidy_data", "bongo2019_emof_study2.csv"))
```
