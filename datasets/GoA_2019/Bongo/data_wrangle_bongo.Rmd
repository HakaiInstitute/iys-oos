...
# Unfortunately trochophore can not be narrowed down further than 'Animalia' [as per Natalie M.]
...
W have to manually change records where length is reported in a range (e.g. 16-20mm) or in an approximation (>4mm). Additionally, we add `occurrenceRemarks` to the records where number is reported as ~3 or >3, to indicate the uncertainty in count. 
...
- [ ] TO DO: Split sample Z15 into multiple rows with unique `occurrenceID`. No individual length or weight measurements were recorded, but observed numbers have been documented and reported in the notes. 
...
Furthermore, for observations of Animalia where length is measured, this represents a minimum length value. These are organisms that were filtered out using a specific sieve. These sieves were 4.0, 2.0, 1.0, 0.5 and 0.25. Therefore, if a length of 0.25 is reported, this is the minimum length, and the maximum length of that species will be 0.5 (any bigger and it would have been filtered out by the larger sieve size). 
...
sieve1 <- sieve %>%
  mutate(minimumLength = ifelse(length == ">4mm", 4, sieve$minimumLength)) %>%
  filter(!is.na(minimumLength)) %>%
  mutate(length = NA)
...
sieve2 <- sieve %>%
  ...
  mutate(maximumLength = case_when(
    length == 0.25 ~ 0.5,
    length == 0.50 ~ 1.0,
    length == 1.00 ~ 2.0,
    length == 2.00 ~ 4.0)) %>%
...
sieve <- rbind(sieve1, sieve2)
sieve_occs <- sieve$occurrenceID

temp2 <- bongo_emof2[ !bongo_emof2$occurrenceID %in% sieve_occs,]
bongo_emof2 <- rbind(sieve, temp2)
```

So now we have a dataframe where:
    * Species have been identified to a species or genus level
    * Length measurement ranges have been split up into columns of minimumLength and maximumLengt where applicable.
    
To fit this into DwC format, all columns but 'measurementRemarks' need to be pivotted to fit a long format:
...
bongo2019_net2_emof <- merge(bongo_emof2, bongo_measurementRemarks,
                      by = c("eventID"), all = T)
