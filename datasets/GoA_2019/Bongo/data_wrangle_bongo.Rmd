...
When filtering for number > 1, we noticed that there are four data entries (rows) with a similar sample_id ("Petri dish"). So in our current set up these all have the same `sample_id` (GoA2019_Stn11:cast1:bongo:Net1:Petri dish). We will deal with these entries separate. So our first step here is to filter out this sample_id and assign a unique `eventID`. However, unique length measurements are provided in the notes for only three of these data entries.
