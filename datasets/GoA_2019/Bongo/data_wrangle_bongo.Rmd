...
Read in the datasets from your local drive. To connect additional metadata information regarding the bongo set deployments, we join another sheet from a different dataset. The dataframe bongo will include the sample_id as well, whereas the bongo_metadata dataframe does not include this information. 
...
The bongo_occ_fnl dataframe is initially saved, and columns are manually added for `kingdom`, `phylum`, `class`, `order`, `family` and `genus`, where applicable. This is done manually because I don't know if it's possible to auto-populate these columns from the WoRMS site directly. Afterwards, this new dataframe is read in. 
...
# Save this dataframe locally so that additional columns on species taxonomic rank can be added manually:
