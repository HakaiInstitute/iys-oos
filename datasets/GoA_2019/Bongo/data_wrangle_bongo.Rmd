...
bongo_cast <- bongo %>%
  ...
  mutate(samplingProtocol = "vertical Bongo Net deployment") %>%
  
  # Ideally samplingProtocol would link the DOI of the paper. 
  
  distinct(eventID, .keep_all = TRUE)
bongo_cast <- bongo_cast %>%
  mutate(eventRemarks = "cast",
         footprintWKT = paste("POINT", " (", bongo_cast$decimalLongitude,
                              " ", bongo_cast$decimalLatitude, ")"))
...
# sampleSizeUnit and sampleSizeValue are included in the eMOF core
# Ideally, the samplingProtocol also includes a DOI of the paper that includes the sampling protocol. 
