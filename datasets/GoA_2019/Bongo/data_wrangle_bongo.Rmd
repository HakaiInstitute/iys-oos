...
Not sure if this means that we have to create two distinct extended measurementOrFact tables which we join to a single eMoF Core? One linking to the `occurrenceIDs` and the other directly to the `eventID`. We first create the eMoF table that links directly to the `eventID`:  
