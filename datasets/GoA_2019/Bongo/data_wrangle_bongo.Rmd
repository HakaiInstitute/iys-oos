...
# No Bongo sets were conducted at station 51 and 52 due to bad weather, so not coordinate information was collected (reflected as 'ND'). Therefore, these need to be removed before calculating the associated coordinateUncertaintyInMeters. However -- in the final event core they need to be added, along with associated notes/eventRemarks. 
...
Q2: When examining samples SS177, SS178 and SS82, it appears that these data entries have a life stage listed in the species column (hydromedusae and crab zoea). Should we manually make changes in the data to reflect this?
</div>

**A**: For Q2, these changes have indeed been made in the script in a later stage to reflect that these species reflect a lifestage, rather than a species.

When examining the bongo_occ_spp dataset, numerous data entries have the sample_id 'Petri dish' associated to them, with the only distinguishable feature being a difference in length measurement. In our current set up, all Petri dish sample_ids belonging to the same station have the same sample (e.g. GoA2019_Stn10:cast1:bongo:Net1:Petri dish), although they are different data entries. Therefore we attach a unique sample to them and then merge them back to the dataframe. Make sure that the value reported in the `number` column is numeric. 
...
When examining the remaining data entries in the bongo_occ_spp dataset, we can identify some problems, especially in the columns `number` and `length_mm`. The `number` column indicates the count of a species observed in each cast, and `count` and `number` will be used synonymous throughout this script. Some problems that we encountered are the following: 
...
# Upon inspection, bongo_occ3 has sample_id 22003 and Z372 as well. These is a samples where number = 1, but the length measurement is a range. Therefore, these samples are already included in bongo_occ3 dataset and are therefore omitted from this dataset. 
...
The bongo_occ_fnl dataframe is saved, and columns are manually added for `kingdom`, `phylum`, `class`, `order`, `family` and `genus`, where applicable. This is done manually because I don't know if it's possible to auto-populate these columns from the WoRMS site directly. Afterwards, this new dataframe is read in. 
...
Next, this Occurrence Core is saved locally, and manually the columns for kingdom, phylum, class etc are added. Afterwards, this dataframe is opened in R again: 
...
bongo_occ_fnl <- read_excel(here("Bongo", "tidy_data", 
  ...
  mutate(lifeStage = case_when(
      ...
      # There is currently no controlled vocabulary for whether a species is dead or alive, but they are currently working on it: https://www.tdwg.org/community/osr/how-did-it-die/ 
      ...
      # Additionally, for two data entries the scientificName is actually more a lifeStage then an actual species, so we record the life stage for these as well:
...
For OBIS, it is preferred if the Event Core and Occurrence Core are combined into a single Core. As you can see, a lot of `eventID` do not have an associated `occurrenceID`, and this is mostly because these `eventIDs` are for zooplankton, which will have measurements directly linked to the `eventID`. 
...
bongo_sampling <- bongo_fnl %>%
  ...
  measurementUnitID = case_when(
    ...
    select(eventID, measurementID, measurementType, measurementTypeID, 
           measurementUnit, measurementUnitID)
```

And for the sampling instrument: 
...
bongo_instrument <- bongo %>%
  ...
  distinct(eventID, .keep_all = TRUE) %>%
  mutate_all(as.character) %>%
  pivot_longer(cols = gear:bongo_length,
               names_to = "measurementType",
               values_to = "measurementValue") %>%
  mutate(measurementID = paste(eventID, "bongo", measurementType, sep = ":"),
         measurementTypeID = case_when(
...
bongo_emof <- bongo_emof %>%
  ...
  mutate(
    measurementID = case_when(
      ...
      # Not really happy with the nomenclature for bongo_opening
    ),
    measurementTypeID = case_when(
...
<div class = "blue">
Q: For sampleID SS146, the life stage is `gravid female`. Should I create column with `sex` for this entry, until a time that there is controlled vocabulary found for this entry?
</div>
...
bongo_emof2 <- bongo_emof2 %>%
  ...
  mutate(
    ...
    measurementValueID = case_when(
      ...
      measurementValue == "2" ~ "http://vocab.nerc.ac.uk/collection/S11/current/S1165/",
      measurementValue == "4" ~ "http://vocab.nerc.ac.uk/collection/S11/current/S1165/",
      measurementValue == "1to2" ~ "http://vocab.nerc.ac.uk/collection/S11/current/S1165/",
...
bongo_measurementOrFact <- bind_rows(bongo_bottomdepth, bongo_sampling, bongo_instrument, bongo_emof, bongo_emof2) %>%
