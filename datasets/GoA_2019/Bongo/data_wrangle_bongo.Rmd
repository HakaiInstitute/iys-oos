...
Read in the datasets from your local drive. To connect additional metadata information regarding the bongo set deployments, we join another sheet from a different dataset. 
...
bongo_date <- bongo_metadata <- read_excel(here("Bongo", "raw_data",
                                  "IYS cruise logbook_2019.xlsx"),
...
# No Bongo sets were conducted at station 51 and 52 due to bad weather, so not coordinate information was collected. Therefore, when calculating decimalLat and long, and the associated coordinateUncertaintyInMeters, these need to be removed. However -- in the final event core they need to be added, along with associated notes/eventRemarks. 
...
bongo_event <- bind_rows(bongo_cruise,bongo_station,bongo_cast) %>% 
  select(eventID, parentEventID:samplingProtocol, eventRemarks) %>%
...
Two columns that might still need to be added to the Event Core: `sampleSizeValue` and `sampleSizeUnit`. The sampling protocol for the Bongo measurements is described [here](https://npafc.org/wp-content/uploads/Public-Documents/2019/1858Prof-Kaganovskiy-Cruise-Summary.pdf). In the [DwC terms](https://dwc.tdwg.org/terms/) it is stated that the definition of the samplingProtocol is "the name of, reference to, or description of the method or protocol used during an Event". For this data set that would be: Bongo net(s) (0.5 m2, mesh size = 236 microns) was deployed to a depth of 250m and towed up vertically. Net 1 of the Bongo was preserved in a 4% formaldehyde seawater solution, net 2 was size fractioned. All net 2 samples were immediately transferred to a -40C blast freezer, and 12 hours later to a -40C storage freezer. 

`samplingProtocol` = "Vertical deployment of Bongo net" - alternatively this could be the link to the Cruise Summary, or DOI once the paper is published?
`samplingEffort` = volume of water filtered? Time (length) of tow? Answer: volumen of water filtered, though I still need to receive (raw) data of the flowmeter attached to the Bongo net. 
...
bongo_occ_core <- bongo_occ_fnl %>%
  ...
  select(eventID, occurrenceID, scientificName, scientificNameID, occurrenceStatus, Preparations,
         organismQuantity, organismQuantityType, stage, lifeStage, sex, reproductiveCondition, occurrenceRemarks)
...
Add information on the sampling instrument used:
...
bongo_instrument <- bongo %>%
  ...
  pivot_longer(cols = gear:bongo_opening,
               names_to = "measurementType",
               values_to = "measurementValue") %>%
  mutate(measurementID = paste(eventID, "bongo", sep = ":"),
...
bongo_instrument$measurementType <- sub("gear", "Sampling instrument name", bongo_instrument$measurementType)
bongo_instrument$measurementType <- sub("bongo_mesh", "Sampling net mesh size", bongo_instrument$measurementType)
bongo_instrument$measurementType <- sub("bongo_opening", 
                                        "Sampling net opening size", bongo_instrument$measurementType)

bongo_instrument <- bongo_instrument %>%
  mutate(measurementUnit = case_when(
           measurementType == "Sampling net mesh size" ~ "um",
           measurementType == "Sampling net opening size" ~ "m^2"),
         measurementUnitID = case_when(
...
bongo_measurementOrFact <- bind_rows(bongo_bottomdepth, bongo_instrument, bongo_emof, bongo_emof2) %>%
