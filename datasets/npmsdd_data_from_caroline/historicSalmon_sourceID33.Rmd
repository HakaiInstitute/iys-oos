...
source_id_33 <- source_id_33 %>%
  ...
  mutate(eventDate = paste(eventDate, year_min, sep = "/")) %>%
...
The minimumDepthInMeters and maximumDepthInMeters is _NA_, and no information pertaining to the fishing depth was found in the paper, so we'll just leave it.

Next we need to create the Event Core. Required columns are: `eventDate`, `eventID`. We also need to figure out how to add coordinates to the Event Core, especially given that for these data entries the coordinates are those of a line string. For all data entries, only the `lat_min`, `lat_max` and `lon_min` are populated, and these will therefore have to correspond to the OBIS columns of `decimalLatitude` and `decimalLongitude`. Additionally, as per OBIS standards, latitude has to be in the range of -90 to 90, inclusive, and longitude has to be in the range of -180 to 180, inclusive. 
...
Finally, as the data is collected in a linestring, OBIS will want to know what the `coordinateUncertaintyInMeters` is. Thankfully there is a neat package and function for this: `obistools::calculate_centroid()`. The function calculates the coordinate uncertainty from a single point (centroid) along the linestring - please be aware that therefore the uncertainty can be quite big!
...
# Only two lon_min values are in this source ID, making it easy to use the ifelse() function.
...
The resourceRelationshipID follows the format of e.g. `database - source_id - predator_id - predatorprey1` etc, the same format as used in the CSIRO sef diet dataset. Some salmonid individuals have more than 1 identified prey spp in their stomach. **However, I haven't found out yet whether uploading the resourceRelationship core through the IPT is needed for OBIS. Instead, currently we have opted to include a column `associatedTaxa` to highlight the relationship. But I would suggest to still make the resourceRelationship core in case workflow changes.**
