...
This dataset concerns aggregated data from the months June and July of several years. Darwin Core follows ISO 8601-1:2019 in this instance, so the `eventDate` should likely read e.g. `1994-06/07`, indicating that the aggregated data was collected across two months in 1994. 
...
Next we need to create the Event Core. Required columns are: `eventDate`, `eventID`. We also need to figure out how to add coordinates to the Event Core, especially given that for these data entries the coordinates are those of a line string. For all data entries, only the `lat_min`, `lat_max` and `lon_min` are populated, and these will therefore correspond to the OBIS columns of `decimalLatitude` and `decimalLongitude`. Additionally, as per OBIS standards, latitude has to be in the range of -90 to 90, inclusive, and longitude has to be in the range of -180 to 180, inclusive. 
...
predator <- source_id_33 %>%
  ...
  select(eventID, decimalLatitude, decimalLongitude, footprintWKT, eventDate,
         year, predator_lowest_taxonomic_level, predator_maturity, predator_sex,
         predator_replicates, coordinateUncertaintyInMeters)

prey <- source_id_33 %>%
  mutate(eventID = paste("npmsdd", source_id, predator_id, "p", sep = "-")) %>%
  select(eventID, decimalLatitude, decimalLongitude, footprintWKT, eventDate, coordinateUncertaintyInMeters,
         year, Actinopterygii:Pteropoda) %>%
...
HS_33 <- full_join(predator, prey1)
hs_33_event <- HS_33 %>% select(eventID, eventDate, year, 
                          decimalLatitude, decimalLongitude, footprintWKT, coordinateUncertaintyInMeters)

# To get the order right:

order2 <- stringr::str_sort(hs_33_event$eventID, numeric = TRUE)
hs_33_event <- hs_33_event[match(order2, hs_33_event$eventID),]
...
hs_33_resourcerelationship <- cbind(id, resourceRelationship, resourceID, relatedResourceID, relationshipOfResource) %>% as.data.frame()
...
prey_occ <- HS_33 %>% filter(!is.na(prey_id)) %>%
...
predator_occ <- HS_33 %>% filter(is.na(prey_id)) %>%
...
``` {r occ}
hs_33_occ <- plyr::rbind.fill(prey_occ, predator_occ)

# To re-order the eventID, use following code:
order <- stringr::str_sort(hs_33_occ$eventID, numeric=TRUE)
hs_33_occ <- hs_33_occ[match(order, hs_33_occ$eventID),]
