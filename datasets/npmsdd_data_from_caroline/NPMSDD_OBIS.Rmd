...
# Bind rows and remove eventID because it's an Occurrence Core. 
NPMSDD_occ <- bind_rows(prey_occ, predator_occ) %>% select(-eventID)
...
occdate <- NPMSDD_occ %>%
  select(occurrenceID, year_min, year_max, warm_cool_years, odd_even_years, season_min, season_max,
...
NPMSDD_occ <- left_join(NPMSDD_occ, date, by = "occurrenceID") %>%
  select(occurrenceID, eventDate, verbatimEventDate, year, month, decimalLatitude, decimalLongitude, footprintWKT, coordinateUncertaintyInMeters, scientificName, scientificNameID, occurrenceStatus, kingdom, phylum, class, order, family, genus, species, gear_type_predator_id1, gear_type_predator_id2, gear_type_predator_id3, gear_type_predator_id4, gear_type_predator_id5) %>%
