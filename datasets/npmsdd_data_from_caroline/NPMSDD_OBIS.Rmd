...
# Step 3: Seasons. Of the remaining sourceIDs, no month_min or month_max should have been provided, but only a season. The first step is to confirm this. As there is no controlled vocabulary for seasons in DwC, these entries won't be listed under eventDate but under verbatimEventDate. Season_max is always NA so this column does not get included in the selection. 

# The only problem here is that eventID "NPMSDD_sourceID:1" contains two entries where only a year is recorded, and then three entries where season is also included. How to deal with this?

seasons <- anti_join(eventdate, dates, by = "eventID")
seasons <- anti_join(seasons, months_final, by = "eventID")

seasons <- seasons %>% select(eventID, year_min, year_max, season_min) %>% group_by(eventID) %>%
  mutate(season_min = case_when(
    season_min == "spring" ~ 41,
    season_min == "summer" ~ 42,
    season_min == "autumn" ~ 43
  )) %>%
  mutate(
    max = ifelse( !all(is.na(season_min)), max(season_min, na.rm = TRUE), NA),
    max_year = ifelse( !all(is.na(year_max)), max(year_max, na.rm = TRUE), NA),
    min = min(season_min, na.rm = TRUE))

seasons_single <- seasons %>% filter(eventID == "NPMSDD_sourceID:1") %>% 
  mutate(verbatimEventDate = paste(min, max, sep = " - "),
         verbatimEventDate = paste(verbatimEventDate, min(year_min), sep = ", "),
         verbatimEventDate = paste(verbatimEventDate, max(year_min), sep = "/"))
seasons_range <- seasons %>% filter(eventID == "NPMSDD_sourceID:14") %>%
  mutate(verbatimEventDate = paste(min, min(year_min), sep = " - "),
         verbatimEventDate = paste(verbatimEventDate, max_year, sep = "/"))
seasons_year_range <- seasons %>% filter(eventID == "NPMSDD_sourceID:28") %>%
  mutate(verbatimEventDate = paste(min, max, sep = " - "),
         verbatimEventDate = paste(verbatimEventDate, min(year_min), sep = ", "),
         verbatimEventDate = paste(verbatimEventDate, max(year_min), sep = "/"))

seasons_final <- bind_rows(seasons_single, seasons_range, seasons_year_range) %>% 
  select(eventID, verbatimEventDate) %>% distinct()
seasons_final$verbatimEventDate <- gsub('41', 'spring', seasons_final$verbatimEventDate)
seasons_final$verbatimEventDate <- gsub('42', 'summer', seasons_final$verbatimEventDate)
seasons_final$verbatimEventDate <- gsub('43', 'autumn', seasons_final$verbatimEventDate)


date <- plyr::rbind.fill(dates, months_final, seasons_final)
  
left_overs <- anti_join(eventdate, date, by = "eventID") # this shows all source_ids have an associated date/time. 

date$year <- ifelse(is.na(date$year_max), date$year_min, NA)
date$month <- ifelse(is.na(date$month_max), date$month_min, NA)
