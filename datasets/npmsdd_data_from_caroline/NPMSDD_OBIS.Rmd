...
source <- read_csv(here::here("datasets", "npmsdd_data_from_caroline", "diet_final.csv"), col_types = "iiiiccccccccttnnnncccciiiiiicccnnnnnnicciciiiiicccccccccccnccccnnnnnnc")
...
gear <- source %>%  select(source_id, gear_type_predator_id1, gear_type_predator_id2,
  ...
  distinct() %>%
  mutate(gear_type_predator_id = as.integer(gear_type_predator_id),
...
Each source_id will have its own associated decimalLatitude and decimalLongitude, along with uncertaintyCoordinatesInMeters and footprintWKT. Different source_ids have their coordinates recorded in varying granularity. In the database there are columns for lat_min, lat_max, lon_min and lon_max. If only lat_min and lon_min are provided, this is a `Point`. If only a single lon_min and lat_min is provided for the source_id, this will be a `Point`. If each data entry within the source_id has a lon_min and lat_min, _but no lon_max or lat_max_, it will create a `Multipoint`. If either lat_max or lon_max is missing, this is a `Linestring`. If all columns are filled out for a specific row/source_id, this signals a `Polygon`. 


Perhaps the way forward is only creating an Occurrence Core, identifying the prey
...
NPMSDD_occ <- left_join(npmsdd, pred_bio, by = "predator_id") %>%
  ...
  mutate(eventID = paste("NPMSDD_sourceID", eventID, sep = ":"),
         ...
         occurrenceID = paste(eventID, predator_id, sep = ":predID"),
         occurrenceID = paste(occurrenceID, prey_lowest_taxonomic_level, sep = ":"),
...
prey_occ <- NPMSDD_occ %>%
    dplyr::select(eventID, lat_min, lat_max, lon_min, lon_max, year_min, year_max, 
                            warm_cool_years, odd_even_years, season_min, season_max,
                            month_min, month_max, date_min, time_min, time_max,
                scientificName, scientificNameID, occurrenceStatus, occurrenceID,
                prey_kingdom, prey_phylum, prey_class, prey_order, prey_family, prey_genus, prey_species,
                gear_type_predator_id1, gear_type_predator_id2, gear_type_predator_id3, gear_type_predator_id4,
...
coordinates <- NPMSDD_occ %>%
  select(eventID,lat_min, lat_max, lon_min, lon_max) %>%
  mutate(footprintWKT = " ") %>%
...
months <- rbind(months_single, month_range, year_range, month_year_range)
...
seasons <- rbind(season_single_year, season_years)
...
NPMSDD_occ <- left_join(NPMSDD_occ, date, by = "occurrenceID") %>%
  select(eventID, occurrenceID, eventDate, verbatimEventDate, year, month, decimalLatitude, decimalLongitude, footprintWKT, coordinateUncertaintyInMeters, scientificName, scientificNameID, occurrenceStatus, kingdom, phylum, class, order, family, genus, species) %>%
  mutate(basisOfRecord = "HumanObservation")

# Add additional column 'associatedTaxa' to indicate that species are prey of others. 
