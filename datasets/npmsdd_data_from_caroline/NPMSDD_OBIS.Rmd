...
eventdate$eventDate <- ifelse(eventdate$min == eventdate$max, eventdate$min, paste(eventdate$min, 
                                                                                    eventdate$max, sep = "/"))

  mutate(eventDate = ifelse(min(dates$date_min) == max(dates$date_min), min(dates$date_min), 
                            paste(min(dates$date_min), max(dates$date_min), sep = "/")))
for (i in 1:nrow(dates)) {
  dates$eventDate[i] <- ifelse(min(dates$date_min) == max(dates$date_min), min(dates$date_min),
                            paste(min(dates$date_min), max(dates$date_min), sep = "/"))
...
for (i in 1:nrow(eventdate)) {
  if(is.na(eventdate$year_max) & is.na(eventdate$month_min) & is.na(eventdate$month_max)
     & is.na(eventdate$season_min) & is.na(eventdate$season_max)) {
    eventdate$eventDate <- eventdate$date_min
  } else if (is.na(eventdate$date_min)) {
       eventdate$eventDate <- NA
     }
}

x <- eventdate %>% group_by(eventID) %>% filter(is.na(date_min))

  
for (i in 1:nrow(eventdate)) {
  if(is.na(eventdate$min)) {
    eventdate$eventDate <- NA
  } 
}
...
eventdate$eventDate <- ifelse(eventdate$min == eventdate$max, eventdate$min, paste(eventdate$min, 
                                                                                    eventdate$max, sep = "/"))
eventdate <- eventdate %>% ungroup(eventID)
