...
Data collected by the 2019 research cruise is currently being processed and reviewed by expedition team members. The goal is to have data centrally accessible in an International Year of the Salmon Ocean Observation System. Our first step was collecting metadata from the expedition Scientists. Next, we aim to publish metadata records in a catalogue which will make the data sets 'Findable'. After due process, data sets will be made 'Accessible' from a metadata catalogue record by providing a link to download data. We conducted a thorough review of the 2019 data we had access to, which was roughly half, and noted issues and missing metadata. We also developed a Dataset Submission Form for expedition data holders (see link below) to upload data sets with a complete suite of metadata that describes collection protocols so critical for data integration and standardization. 

The core components of the Data Management and Communications model include: 1) a Data Catalogue that centralizes data access through a web portal; 2) Catalogue records compliant with ISO 19115 that make the data sets broadly discoverable; 3) Open-access licensing; 4) Open Data Access Protocols, specifically ERDDAP data servers, where necessary and; 5) Controlled Vocabularies that define the variables, methods, units, and measurement types used in salmon ocean ecology adhering to ‘Ocean Best Practices’ maintained by the Global Ocean Observation System. 
...
A number of data sets were delivered to the Hakai Institute from holdings that Drs. Evgeny Pakhomov and Brian Hunt had access to. A Google Drive folder was set up to hold data for the short-term. To receive data more efficiently,  a [Dataset Submission Google Form](https://forms.gle/BvMCLTGSjKsm42cS7) and a [static Word doc]("reports/IYS-GoA Dataset Submission Form.docx"). form were created to capture metadata. A number of these need to be filled out still (Table 1.). We’ve looked through all of the 2019 data we have access to at this time, which is less than half of the expected data sets. We noted issues and missing metadata for the 2019 data and are now soliciting feedback and collaboration with expedition scientists. Please contact us with new information on the status of data sets listed missing in Table 1.
