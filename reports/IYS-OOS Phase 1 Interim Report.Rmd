...
# _International Year of the Sal mom Data Mobilization Interim Report_
...
* Communicate. Representatives from each scientific discipline involved with the IYS should connect through a working group that disseminates and advocates for best-pracitces.
...
The development of the data model will rely heavily on input from expedition Chief Scientists, Scientists, project affiliated Scientists, Professors, and others. How exactly teams divide up development tasks will become clearer after tasks are better defined as a result of community engagement.
...
src <- c( "Aleksey Somov",
          ...
          "Stephanie Taylor",
          "Suam Kim",
          "Sventlana Eukenselova",
          "Terry Beacham",
          "Vishnu Perumthuruthil Suseelan",
          "Vladmir Radchenko")

target <- c("TINRO Russian Pacific Federal Fisheries Research Institute",
"UBC University of British Columbia, IOF Institute of Oceans and Fisheries, Institute of Oceans and Fisheries",
"PSF Pacific Salmon Foundation",
"NPAFC North Pacific Anadromous Fisheries Commission",
"NOAA National Oceanic and Atmospheric Administration",
"DFO Fisheries and Oceans Canada, UBC University of British Columbia",
...
"Japan National Fisheries Research and Education Center",
"SalmoForsk",
"NPAFC North Pacific Anadromous Fisheries Commission",
"NPAFC North Pacific Anadromous Fisheries Commission",
"PSF Pacific Salmon Foundation",
...
# Read from Air Table
download.file("https://airtable-csv-exports-production.s3.amazonaws.com/c1f585fb575d029a/People-Grid%20view.csv?AWSAccessKeyId=AKIAR7KB7OYSMPA2CKCB&Expires=1585847543&Signature=CPbyYK%2FIfnGe069JGc%2BscVRSwHI%3D", destfile = here::here("data", "people.csv"))

people <- read_csv(here("data", "people.csv")) %>% 
  select(Name, Organization) %>% 
  as.data.frame()

forceNetwork(Source = people, source = Name, Target = Organization, Links = NULL)
...
Here is a possible road map and timeline which will be refined before finalization for May 30, 2020.
...
We need to better capture methods used in the field in the metadata associated with the measurements made on the expedition. This includes platform  descriptions, equipment descriptions, calibration files, and expected key variables that will be delivered. This will help in ensuring the scientific integrity of the consolidated data sets. Dataset 'Quality' levels will be assigned based on completeness of metadata collection and integrity of provenance. Development of these guidelines is pressing and there is need to draw together specific domains to discuss these issues.

Trawl catch data has not yet been received. Current understanding indicates that the  dataset structure is under development, and that expedition Scientists are working on this. Please understand that access to raw outputs and the detailed processing steps that occur to transform data are needed for complete data provenance. Keeping track of changes to raw data is important for reproducibility, and becoming commonly required in similar life sciences journals despite it being a high bar to aim for. Using a change log that you maintain manually ensures that reproducibility can be achieved when data cleaning is performed ad hoc such as removing outliers during quality control procedures, etc. Otherwise, using automated version control, scripted data transformations and distributed version control help scientists collaborate on common data and analyses.

We recommend the adhoc development of data standards group comprised of representatives of each research domain (physical oceanography, salmon feeding ecology etc.) and representation from as many member nations as possible. 

# Next Steps

Produce a Data Strategy Document
This will:
...
The Global Ocean Observing System 2030 Strategy. IOC, Paris, 2019, IOC Brochure 2019-5 (IOC/BRO/2019/5 rev.2), GOOS Report No.239
