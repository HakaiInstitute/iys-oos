...
For the IYS project, all the raw data for the collected data gets stored on GoogleDrive, so that it is publicly accessible. The raw data can be found [here](https://drive.google.com/drive/u/0/folders/0AHNahA4cmbSJUk9PVA). It is also in these Google Drive folders that we store the tidy versions of the data, in which the data is formatted and standardized based on the Darwin Core (DwC) standard. Moving forward, we might use a different data repository, such as GitHub, to host both the raw and the formatted data.

## 2. GitHub

Few important links to get started with GitHub, and linking it to R, the statistical computing language that we typically use for analyses:
...
Below are listed some important command lines within GitHub that can be useful to e.g. push, pull or clone a repository locally or to the remote. The Hakai Institute public IYS-OOS repository (`repo`) can be found [here](https://github.com/HakaiInstitute/iys-oos). This repo consists of a `master` branch, a `develop` branch, and multiple `feature` branches. The practice currently adopted is that we write the .R and .Rmd files for each separate dataset in these `feature` branches, along with a changelog (.Rmd file). In the changelog we record any changes made to the raw data, and what the output files are for each transformed dataset. 

__Please note: We should try and limit any changes made to the raw data. It is important to record any changes made to the raw data in the changelog file, ideally with a reason as to why it was done. If any major changes are made to the raw data (i.e., adding columns), it is suggested that the raw data then be saved under a different file name and work from that.__

The RMarkdown (.Rmd) script contains the code used to transform or 'tidy' the data to follow Darwin Core standards, along with a more thorough description of why certain functions were used. The .Rmd and changelog files get saved in the IYS-OOS repo in GitHub, whereas the tidied data get saved on Google Drive with a .csv extension. 

When working with GitHub, the first step is to `clone` the repository you'll be working from. A clone is basically a duplicate of another repository, getting your own copy of someone else's code. The command line for this is `$ git clone [...]` e.g.: 

> $ git clone https://github.com/HakaiInstitute/iys-oos.git
>

This clones the master branch and all associated develop and feature branches. A good practice is to create a `feature` branch on which you can work without directly affecting the `develop` or `master` branch. A new `feature` branch can be created in RStudio but also through a command line in Git. **It is important to create a new branch off of the correct branch! For example, don't create a feature branch that branches off from another feature branch.** Branches can be created locally as well as on the remote. To create a new feature branch locally (e.g. feature/ctd, on which the CTD data gets wrangled), the command line is: 
...
The checkout command lets you navigate and switch between the branches. Checkout works to switch between versions of code already on the local system. __Important: when you make changes in an R script that nests under a specific feature branch (e.g. CTD-wrangling script under feature/ctd), make sure to commit and push this first before switching to another (feature) branch to work on a different R script. Otherwise changes you made to the CTD .Rmd script will also be committed and pushed onto a subsequent branch you're working on.__ To confirm you are now working on the correct branch: 

> $ git branch
>

Once you have created a new (feature) branch locally, you can now freely work in this branch. Any saves you make will be saved to your local workstation, _not_ to GitHub. To save the changes to the remote, the first step is to `commit` your changes. This can be done through RStudio, or alternatively, by using the following command line:
...
Please note that this commits your changes, they are not yet 'pushed' to GitHub. The 'commit' command is used to save your changes to the local repository. When you commit, make sure to add a message so that your collaborators know what changes you have made to the file. You can make as many commits as you want, but it is recommended to only push once or twice a day (right?). By pushing your commits, they will be saved on GitHub. You can push your commits through RStudio, or using:

> $ git push <repo name> <branch name>
>

So if we've been working on the .Rmd script which we want to push to the feature/ctd branch of our iys-oos repo, the syntax would be:

> $ git push origin feature/ctd
>

The `origin` is a shorthand name for the remote repository that a project was originally cloned from (in this case, iys-oos). More precisely, it is used instead of that original repository's URL, making referencing much easier. Once pushed, the commits made to the scripts can be found on GitHub, and then be 'pulled' by other collaborators if they wish to make contributions or modifications to the code. Any changes that have been saved with 'commit', but have not been pushed to GitHub, cannot be pulled by other collaborators. A `pull` is useful to make sure you have the latest modifications / version from GitHub, and keep your code updated with whatever changes may have been applied to the original. 
...
__When naming branches, files and folders and committing/pushing it to the remote, it is good practice to make sure these are as concise as possible, and hyphenated__.
...
The issues can also be `labeled` (i.e., "help wanted", "to do", "question"), which makes it easier to filter for specific labels or categorize them. Multiple labels can be attached to a single issue. You can also assign contributors (`Assignee`) to a specific issue so that they get notified of issues that they need to work on. Additionally, `story points` get attached to an issue. Story points are unitless scales of measurement which are sized in relation to other tasks. The number of storypoints within each column gives an indication how big (and therefore perhaps, as a proxy, how time-consuming) the issues are.
...
The Ocean Biogeographic Information System (OBIS) is a global open-access data and information clearing-house on marine biodiversity for science, conservation and sustainable development. OBIS' mission is to collaborate with scientific communities globally to facilitate free and open access to, and application of, biodiversity and biogeographic data and information on marine life. The datasets integrated to OBIS need to adhere to specific language to make the data interoperable. The main objective of this user manual is to describe steps taken, lessons learned and issues encountered (and, hopefully, overcome!) in wrangling data collected for the International Year of the Salmon (IYS) project into OBIS. The data hosted on OBIS eventually feeds into the Global Ocean Observing System (GOOS), which uses a similar language. By using similar language, the data of the IYS project will be interoperable with other data hosted under the GOOS umbrella. 

Few important links: 
[OBIS Manual](https://obis.org/manual/)
[GOOS](https://www.goosocean.org/)
...
Update __May 26, 2020__: it seems the column `type` in the Event Core has been replaced by `eventRemarks`. Whilst it is still not a required term in the Event table, it can be useful to categorize and structure your data. Controlled vocabulary exists for the `type` column and can be found [here](https://dwc.tdwg.org/terms/).  
...
Some other DwC terms or conventions are 'overruled' by OBIS manual, such as e.g.: `individualCount`, `organismQuantity`, `organismQuantityType`, `sex` and `lifeStage`. For these terms, OBIS recommends that the measurements get added to the eMoF table. However, after discussion with the the Standardizing Marine Biological Data (SMBD) group, it was recommended that these five terms **also** be included in the Occurrence Core. The reason for this is that the data should be shared with both OBIS and GBIF, and typically these users look in the Occurrence Core for this data. However, make sure to add the measurements to the eMoF core also, as the benefits of storing measurements in the eMoF extension are:
...
### extended Measurement or Fact Core
