...
The RMarkdown (.Rmd) script contains the code used to transform or 'tidy' the data to follow Darwin Core standards, along with a more thorough description of why certain functions were used. The .Rmd and ChangeLog file get saved in the IYS-OOS repo in GitHub, whereas the tidied datasets get saved on GoogleDrive with a .csv extension. 
...
This clones the master branch and all associated develop and feature branches. A good practice is to create a `feature` branch on which you can work without directly affecting the `develop` or `master` branch. A new `feature` branch can be created in RStudio but also through a command line in Git. Branches can be created locally as well as on the remote. To create a new feature branch locally (e.g. feature/ctd, on which the CTD data gets wrangled), the command line is: 
...
Once you have created a new (feature) branch locally, can now freely work in this branch. Any saves you make will be saved to your local workstation, _not_ to GitHub. To save the changes to the remote, the first step is to `commit` your changes. This can be done through RStudio, or alternatively, by using the following command line:
...
The `origin` is a shorthand name for the remote repository that a project was originally cloned from (in this case, iys-oos). More precisely, it is used instead of that original repository's URL, making referencing much easier. Once pushed, the commits made to the scripts can be found on GitHub, and then be 'pulled' by other collaborators if they wish to make contributions or modifications to the code. A `pull` is useful to make sure you have the latest modifications / version from GitHub, and keep your code updated with whatever changes may have been applied to the original. 
